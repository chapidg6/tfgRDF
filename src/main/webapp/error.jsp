...
<body>
	...
	<div class="container">
		...
		// Obtener atributos de la solicitud y la sesión
		String fileName = (String) request.getAttribute("fileName");
		String errorCampoRecurso = (String) sessionError.getAttribute("errorMessage");
		String errorResultados = (String) request.getAttribute("erroResultados");
		%>
		<div class="error-box">
			<%
			if (fileName != null) {
			%>
			Error de E/S:
			<%=fileName%>. Es un archivo incompatible.
			<%
			} else if (errorCampoRecurso != null) {
			%>
			<%=errorCampoRecurso%>
			<%
			} else if (errorResultados != null) {
			%>
			<%=errorResultados%>
			<%
			} else {
			...
			sessionError.removeAttribute("errorMessage");
			%>
		</div>
